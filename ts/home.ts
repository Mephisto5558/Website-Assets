...
(function updateText(currentText: string = texts[0], currentColor: string = colors[0], isAdding = true): NodeJS.Timeout {
  ...
  if (isAdding) {
    ...
    if (textElement.textContent.length >= currentText.length) return setTimeout(() => void updateText(currentText, currentColor, false), msInSecond);
  }
  else {
    textElement.textContent = textElement.textContent.slice(0, -step);
    if (textElement.textContent.length <= 0) return setTimeout(() => void updateText(...getTextAndColor.next().value, true), msInSecond);
  }

  return setTimeout(() => void updateText(currentText, currentColor, isAdding), updateTimeoutMs);
})();